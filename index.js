...
function addStar() {
  ...
  star.position.set(Math.random()*400 - 900, Math.random()*400 -900, z -900);
  scene.add(star);
}
...
Array(800).fill().forEach(addStar);
  ...
  const spaceTexture = new THREE.TextureLoader().load('newlook/assets/sky5.jpg');
...
// Pluto
  const plutoTexture = new THREE.TextureLoader().load('newlook/assets/pluto.webp');
  const pluto = new THREE.Mesh(new THREE.SphereGeometry(0.6, 64, 64), new THREE.MeshBasicMaterial({ map: plutoTexture }));
  scene.add(pluto);
  pluto.position.x = -6;
  pluto.position.y = -4;
  pluto.position.z = 30;
...
// Sun
  ...
  var orbit_pluto = 1;
  var pluto_orbital_speed = Math.random()/100
  var orbit_mars = 5;
  ...
  function animate() {
    ...
    orbit_pluto = animate_orbit(pluto,orbit_pluto,5,pluto_orbital_speed);
